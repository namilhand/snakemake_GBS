...
awk 'BEGIN{IFS="\t"; OFS="\t"}{print "Chr"$2, $3, substr($4, 2, 1), $5, substr($6, 2, 1), $7}' ./from_ian/BC.complete.tiger.txt | tail -n +2 | bgzip > BC.complete.tsv.gz

# The input data file for [tabix] must be position sorted and compressed by bgzip.
tabix -s1 -b2 -e2 BC.complete.tsv.gz
